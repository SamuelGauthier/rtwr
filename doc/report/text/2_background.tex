...
\autoref{subsec:shallow_water} (here we include the intermediate models directly
into the shallow group). For each of them we selected relevant approaches from
the literature that we will present. Readers who desire to get a broader picture
should read the survey from 2011 from Darles et al.:
\autocite{darles2011survey}. Then in \autoref{subsec:ocean_details} we discuss
the methods to produce realistic water surfaces. This includes caustics, foam,
light scattering and more. Finally in \autoref{subsec:candidate_apps} we present
two applications in need of a real-time water solution.
...
are shown below only for the curious reader as we will not further investigate
them. The equation of incompressible fluid flow,
